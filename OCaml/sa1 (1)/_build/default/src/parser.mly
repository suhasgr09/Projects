...
expr:
    ...
    | MAKE_ARRAY; LPAREN; len = expr; COMMA; def = expr; RPAREN { MakeArray(len,def) }
    | SET_ARRAY; LPAREN; arr = expr; COMMA; idx = expr; COMMA; v = expr; RPAREN { SetArray(arr,idx,v) }
    | GET_ARRAY; LPAREN; arr = expr; COMMA; idx = expr; RPAREN { GetArray(arr,idx) }
    | LENGTH_ARRAY; LPAREN; arr = expr; RPAREN { LengthArray(arr) }
