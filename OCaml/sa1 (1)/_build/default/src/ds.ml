...
let run (c:'a ea_result) : 'a result =
  c EmptyEnv
let make (e1,e2) = 
    let rec make' e1 e2 acc = 
      if e1 = e2 then e1::acc
      else make' (e1+1) e2 (e1::acc)
    in make' e1 e2 []
...
let rec make_list_from_array = function
  | [] -> []
  | (ArrayVal l)::t -> l@(make_list_from_array t)
...
let clos_of_procVal : exp_val -> (string*Ast.expr*env) ea_result = function
...
let list_of_arrayVal = function
  | ArrayVal xs -> return xs
...
let rec string_of_expval = function
  ...
  | ArrayVal(xs) -> "ArrayVal([" ^ String.concat "; " (List.map string_of_expval xs) ^ "])"
and
...
and
  string_of_expval_list = function
  | [] -> ""
  | e :: es -> string_of_expval e ^ "; " ^ string_of_expval_list es
