...
(* is_subclass and auxiliary functions *)
    ...
    let is_subclass : string -> string -> class_env -> exp_val ea_result = 
      fun c_name1 c_name2 c_env ->
        ...
        | None -> error ("is_subclass: class c2 not found")
        | Some (_super, methods, fields) ->
          if _super = c_name2 || c_name1 = c_name2 then return @@ BoolVal true
           else return @@ BoolVal(_super = "object")
...
and
  ...
  | IsInstanceOf(e,id) ->
    ...
    obj_of_objectVal self >>= fun(c_name,env) ->
      is_subclass c_name id !g_class_env >>= fun e_r ->
       return@@(e_r)

    
  | Cast(e,id) ->
    eval_expr e >>= fun self ->
      obj_of_objectVal self >>= fun(c_name,c_env) ->
        is_subclass c_name id !g_class_env >>= fun e_r ->
          return @@ ObjectVal(c_name, c_env)
