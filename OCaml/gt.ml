...
─( 04:11:12 )─< command 0 >──────────────────────────────────────────────────────────────────────────────────────────────────────{ counter: 0 }─
utop # #use "gt.ml";;
type 'a gt = Node of 'a * 'a gt list
This is Tree Provided to us in the assignment

val t : int gt =
  Node (33,
   [Node (12, []);
    Node (77, [Node (37, [Node (14, [])]); Node (48, []); Node (103, [])])])

This is the tree I have created to validate against the functions

val t2 : int gt =
  Node (33,
   [Node (12, []); Node (77, []); Node (37, []); Node (48, []); Node (103, [])])
  
This is the Map function which applies the fucntion f to both head and tail of the list

val map : ('a -> 'b) -> 'a list -> 'b list = <fun>
    
This is the mk_leaf function that given n builds a general tree that is a leaf holding n as data.
  
val mk_leaf : 'a -> 'a gt = <fun>
    
BEGIN
1) height: that given a general tree returns its height. The height of a tree is the length of the longest (in terms of number of nodes) path from the root to a leaf.
  
val height : 'a gt -> int = <fun>
  
Testing the height fucntion on both the trees

Test for Tree t
val test_height : unit = ()
- : int = 4
Test for Tree t2
val test_height2 : unit = ()
- : int = 2
END
...
val size : 'a gt -> int = <fun>
 
Testing the SIZE fucntion on both the trees
...
val paths_to_leaves : 'a gt -> int list list = <fun>
 
Testing the PATH_TO_LEAVES fucntion on both the trees

Test for Tree t
val test_path_to_leaves : unit = ()
- : int list list = [[0]; [1; 0; 0]; [1; 1]; [1; 2]]
Test for Tree t2
val paths_to_leaves2 : unit = ()
- : int list list = [[0]; [1]; [2]; [3]; [4]]
END
    
BEGIN
4) is_leaf_perfect: that determines whether a general tree is leaf perfect. A general tree is said to be leaf perfect if all leaves have the same depth.

val is_leaf_perfect : 'a gt -> bool = <fun>
 
Testing the IS_PERFECT_LEAF fucntion on both the trees

Test for Tree t
val test_is_leaf_perfect : unit = ()
- : bool = false
Test for Tree t2
val test_is_leaf_perfect2 : unit = ()
- : bool = true
END
...
val preorder : 'a gt -> 'a list = <fun>
 
Testing the PREORDER fucntion on both the trees

Test for Tree t
val test_preorder : unit = ()
...
val mirror : 'a gt -> 'a gt = <fun>
 
Testing the MIRROR fucntion on both the trees

Test for Tree t
val test_mirror : unit = ()
- : int gt =
Node (33,
 [Node (77, [Node (103, []); Node (48, []); Node (37, [Node (14, [])])]);
  Node (12, [])])
Test for Tree t2
val test_mirror2 : unit = ()
- : int gt =
Node (33,
 [Node (103, []); Node (48, []); Node (37, []); Node (77, []); Node (12, [])])
END
    
BEGIN
7) mapt f t: that produces a general tree resulting from t by mapping function f to each data item in d.

val mapt : ('a -> 'b) -> 'a gt -> 'b gt = <fun>
 
Testing the MAPt fucntion on both the trees
...
- : bool gt =
Node (true,
 [Node (false, []); Node (true, []); Node (true, []); Node (true, []);
  Node (true, [])])
END
...
val foldt : ('a -> 'b list -> 'b) -> 'a gt -> 'b = <fun>
    
SUMt
val sumt : int gt -> int = <fun>
MAPt
val memt : 'a gt -> 'a -> bool = <fun>
...
Node (33,
 [Node (77, [Node (103, []); Node (48, []); Node (37, [Node (14, [])])]);
  Node (12, [])])
Test for Tree t2
val test_mirror'2 : unit = ()
- : int gt =
Node (33,
 [Node (103, []); Node (48, []); Node (37, []); Node (77, []); Node (12, [])])
─( 04:11:12 )─< command 1 >──────────────────────────────────────────────────────────────────────────────────────────────────────{ counter: 0 }─
utop # 
