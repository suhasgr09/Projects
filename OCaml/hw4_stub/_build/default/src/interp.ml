...
and
  ...
  | Proj(e,id) ->
    ...
    let rec findfield: (string * (bool * exp_val)) list -> exp_val ea_result =
    fun x ->
    match x with 
  ...
  | SetField(e1,id,e2) ->
    eval_expr e1 >>= 
    fields_of_recordVal >>= fun v ->
    let rec setfield: (string * (bool * exp_val)) list -> exp_val ea_result =
    fun x ->
    match x with 
    ...
    (if fst (snd h) 
    then 
    int_of_refVal(snd(snd h)) >>= fun l ->
    eval_expr e2 >>= Store.set_ref g_store l >>= fun _ ->
    return UnitVal
  ...
  | IsNumber(e) ->
    eval_expr e >>= fun t  -> 
      is_numVal t >>= fun x -> return (BoolVal x)
  | Unit -> return UnitVal
  | Debug(_e) ->
