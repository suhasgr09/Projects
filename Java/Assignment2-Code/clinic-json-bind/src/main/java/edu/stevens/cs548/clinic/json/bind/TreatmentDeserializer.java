...
import edu.stevens.cs548.clinic.json.bind.TreatmentDeserializer;
...
public class TreatmentDeserializer {
	...
	private TreatmentDto deserialize(JsonElement json)
		...
		if(typeTag.equals(TreatmentType.DRUGTREATMENT)) {
			return gson.fromJson(json, DrugTreatmentDto.class);
			}
			else if (typeTag.equals(TreatmentType.RADIOLOGY))
			{
				return gson.fromJson(json, RadiologyDto.class);
			}
			else if (typeTag.equals(TreatmentType.SURGERY))
			{
				return gson.fromJson(json, SurgeryDto.class);
			}
			else if (typeTag.equals(TreatmentType.PHYSIOTHERAPY))
			{
