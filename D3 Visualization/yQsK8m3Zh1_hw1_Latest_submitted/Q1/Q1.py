...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list=None) -> list:
        ...
        movie_cast = [member for member in cast if member['id'] not in exclude_ids and member['order'] < limit]
    ...
    def get_movie_credits_for_person(self, person_id:str, vote_avg_threshold:float=None)->list:
        ...
        person = [role for role in cast if role['vote_average'] >= vote_avg_threshold]
        return person
...
if __name__ == "__main__":
    api_key = 'e84864613c0cbefbf6bbc35d92f42df8'
    graph = Graph()
    base_graph_nodes = set()
    main_id = '2975'
    ...
    movies = tmdb_api_utils.get_movie_credits_for_person(main_id, 8.0)
    ...
    for movie in movies:
        ...
        for cast in cast_members:
            cast_id = str(cast['id'])
            if cast_id not in base_graph_nodes:
                base_graph_nodes.add(cast_id)
                graph.add_node(cast_id, cast['name'])
            graph.add_edge(main_id, cast_id)
    loop = 2
    while loop > 0:
        base_graph_nodes_new = set()
        for node in base_graph_nodes:
            movie_credits = tmdb_api_utils.get_movie_credits_for_person(node, 8.0)
            ...
            for movie_credit in movie_credits:
                ...
                for cast in cast_members:
                    cast_id = str(cast['id'])
                    if cast_id not in base_graph_nodes:
                        base_graph_nodes_new.add(cast_id)
                        graph.add_node(cast_id, cast['name'])
                    graph.add_edge(node, cast_id)
        base_graph_nodes = base_graph_nodes_new
        loop -= 1
